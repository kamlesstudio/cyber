...
def check_dumpcap():
    ...
    if 'cap_net_admin' and 'cap_net_raw' in caps.decode('utf-8').split('=')[1]:
...
#    out = r('docker ps -aq --filter label=w4sp=true').decode('utf-8').split('\n')[:-1]
#    for c_id in out:
#        r('docker rm -f $c_id')

#    for nic in r('ipconfig -a').decode('utf-8').split('\n\n')[:-1]:
#        nic = nic.split(' ')[0]
...
def docker_clean():
    """Clean up w4sp-related containers and network artifacts"""

    # Connect to Docker daemon
    client = docker.from_env()

    # Remove containers with label w4sp=true
    containers = client.containers.list(all=True, filters={'label': 'w4sp=true'})
    for container in containers:
        container.remove(force=True)

    # Remove network interfaces (except docker0, eth0, lo, and those with 'root')
    interfaces = subprocess.run(['ip', 'link'], check=True, capture_output=True, text=True).stdout.split('\n\n')[:-1]
    for interface in interfaces:
        interface_name = interface.split(' ')[0]
        if interface_name not in ['docker0', 'eth0', 'lo'] and 'root' not in interface_name:
            try:
    ...
    namespaces = subprocess.run(['ip', 'netns'], check=True, capture_output=True, text=True).stdout.split('\n')[:-1]
    for namespace in namespaces:
        subprocess.run(['ip', 'netns', 'delete', namespace], check=True)

    # Kill old dhclients
    try:
        subprocess.run(['pkill', 'dhclient'], check=True)
    except subprocess.CalledProcessError:
        pass
    ...
    for interface in interfaces:
        interface_name = interface.split(' ')[0]
        if 'root' in interface_name:
            try:
                subprocess.run(['ip', 'link', 'set', interface_name, 'down'], check=True)
                subprocess.run(['ip', 'link', 'set', interface_name, 'name', 'eth0'], check=True)
            except subprocess.CalledProcessError:
                pass

    # Restart networking service (you can adjust this based on your system)
    try:
    ...
    subprocess.run(['service', 'docker', 'restart'], check=True)
